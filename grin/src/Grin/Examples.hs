...
add =
  Program
    ...
    [ Def "add" ["s1", "s2"] $
        ...
        Pure (Var "s3")
    , Def "main" [] $
        Bind (Pure (Lit (LVal (SInt64 10)))) (BVar "m1") $
    ...
    [ Def "fact" ["f1"] $
        ...
        Case "f3"
          [ Alt (LitPat (SBool True)) $
                Bind (Pure (Lit (LVal (SInt64 1)))) (BVar "f7") $
                Pure (Var "f7")
          , Alt (LitPat (SBool False)) $
                Bind (Pure (Lit (LVal (SInt64 1)))) (BVar "f4") $
...
sumSimple =
  Program
    ...
    [ Def "main" [] $
        ...
        Bind (Pure (Lit (LVal (SInt64 100)))) (BVar "m2") $
        Bind (Pure (Lit (LNode (Node (Tag C "Int") ["m1"])))) (BVar "m3") $
        Bind (Pure (Lit (LNode (Node (Tag C "Int") ["m2"])))) (BVar "m4") $
        Bind (Store "m3") (BVar "m5") $
        Bind (Store "m4") (BVar "m6") $
        Bind (Pure (Lit (LNode (Node (Tag F "upto") ["m5", "m6"])))) (BVar "m7") $
        Bind (Store "m7") (BVar "m8") $
        Bind (Pure (Lit (LNode (Node (Tag F "sum") ["m8"])))) (BVar "m9") $
    ...
    , Def "upto" ["u1", "u2"] $
        ...
        Case "u5"
          [ Alt (LitPat (SBool True)) $
                Bind (Pure (Lit (LNode (Node (Tag C "Nil") [])))) (BVar "u12") $
                Pure (Var "u12")
          , Alt (LitPat (SBool False)) $
                Bind (Pure (Lit (LVal (SInt64 1)))) (BVar "u6") $
                Bind (App "prim_int_add" ["u3", "u6"]) (BVar "u7") $
                Bind (Pure (Lit (LNode (Node (Tag C "Int") ["u7"])))) (BVar "u8") $
                Bind (Store "u8") (BVar "u9") $
                Bind (Pure (Lit (LNode (Node (Tag F "upto") ["u9", "u2"])))) (BVar "u10") $
                Bind (Store "u10") (BVar "u11") $
                Pure (Lit (LNode (Node (Tag C "Cons") ["u1", "u11"])))
          ]
    , Def "sum" ["s1"] $
        Bind (App "eval" ["s1"]) (BVar "s2") $
        Case "s2"
          [ Alt (NodePat (Tag C "Nil") []) $
                Bind (Pure (Lit (LVal (SInt64 0)))) (BVar "s3") $
                Pure (Lit (LNode (Node (Tag C "Int") ["s3"])))
          , Alt (NodePat (Tag C "Cons") ["s5", "s6"]) $
                ...
                Pure (Lit (LNode (Node (Tag C "Int") ["s9"])))
          ]
    , Def "eval" ["e1"] $
        Bind (Fetch "e1") (BVar "e2") $
        Case "e2"
          [ Alt (NodePat (Tag C "Int") ["e3"]) $
                Bind (Pure (Lit (LNode (Node (Tag C "Int") ["e3"])))) (BVar "e11") $
                Pure (Var "e11")
          , Alt (NodePat (Tag C "Nil") []) $
                Bind (Pure (Lit (LNode (Node (Tag C "Nil") [])))) (BVar "e12") $
                Pure (Var "e12")
          , Alt (NodePat (Tag C "Cons") ["e4", "e5"]) $
                Bind (Pure (Lit (LNode (Node (Tag C "Cons") ["e4", "e5"])))) (BVar "e13") $
                Pure (Var "e13")
