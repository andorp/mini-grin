...
collectNonUsedParams = cata $ \case
  ...
  DefF _funName ps (Names used) -> NonUsedParam $ undefined ps used
  DefF _ _ (NonUsedParam _)    -> error "collectNonUsedParams: DefF with NonUsedParam)"

  -- Exercise: Collect the names from the literal
  SPureF _literal -> Names undefined
  ...
  EBindF _lhs (BNodePat _tag _names) _rhs -> undefined

  ECaseF n alts -> mconcat $ Names (Set.singleton n) : alts
  AltF _ body   -> body

  SAppF _f _xs -> undefined


removeFunctionParams :: Info -> Exp -> Exp
removeFunctionParams (Names _) = id
removeFunctionParams (NonUsedParam funsToChange) = ana $ \case
  Def funName ps body -> case Map.lookup funName funsToChange of
    ...
    Just _paramsToElim -> DefF funName (undefined ps) body
  ...
  SApp funName params -> case Map.lookup funName funsToChange of
    ...
    Just _paramsToElim -> SAppF funName (undefined params)
